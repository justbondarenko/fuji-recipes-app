...
interface RecipeRepository {
    ...
    /**
     * Apply one partial body to several recipes at once — the library's selection mode.
     *
     * One call rather than a loop for the same reason as [importAll]: the library is
     * rewritten whole on every mutation, so N changes as N calls means N file writes and N
     * republished lists, with the list re-laying out under the user's finger between each.
     * The default here is that loop, for implementations that have nothing to gain from
     * batching; the local store overrides it with a single commit.
     */
    suspend fun updateAll(ids: Set<String>, body: JsonObject): LibraryResult<Unit> {
        ids.forEach { id ->
            val result = update(id, body)
            if (result is LibraryResult.Failure) return LibraryResult.Failure(result.error)
        }
