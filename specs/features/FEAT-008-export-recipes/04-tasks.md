...
- [ ] **T-01** — `data/exporting/RecipeExport.kt` ← `shared/format/export.ts`. `EXPORT_EXCLUDED`
...
- [ ] **T-02** — `toExportRecipe`: documented keys in order, unknown keys after them
...
- [ ] **T-03** — `buildEnvelope` + `serialise`: SF-002's envelope in SF-002's key order,
      two-space indent, trailing newline, UTF-8 with no BOM (SF-001).
- [ ] **T-04** — SF-018 filenames: `libraryExportFilename(kind, at)` with a **UTC** date, and
      `recipeExportFilename` — slugified, keeping Unicode letters, falling back to
      `recipe-<id8>.json` when a name slugifies to nothing.
- [ ] **T-05** — `archiveEntries`: one bare recipe per entry, names **de-duplicated** with
      `-2`, `-3`. Recipe names are not unique, and a map keyed by name would silently ship one
      recipe where there were two.
- [ ] **T-06** — `RecipeExportTest`: `recipe-format.spec.md` §3's own example round-trips key
...
- [ ] **T-07** — `data/exporting/ZipWriter.kt`: entries to a `ByteArray` via
      `java.util.zip.ZipOutputStream` (`tech-stack.md` §2 — stdlib, no dependency).
- [ ] **T-08** — `ZipWriterTest`: the archive reads back through `ZipInputStream` with entry
      names and contents intact, and an entry's content parses as a recipe.

## Sharing

- [ ] **T-09** — `core/share/ShareFile.kt`: write to `cacheDir/export/`, wrap in a
...
- [ ] **T-10** — `AndroidManifest.xml` gains the `FileProvider`, and
...
- [ ] **T-11** — `ui/exporting/ExportViewModel.kt`: the library, the selection (**everything
      selected on open**), the JSON/ZIP choice, and the filename the action will produce.
- [ ] **T-12** — `ui/exporting/ExportScreen.kt`: the selection list, select all / none, the
...
- [ ] **T-13** — `ExportRoute` in `ui/nav/FujiNavHost.kt`, a row on
...
- [ ] **T-14** — An **Export** action on `ui/recipe/RecipeViewScreen.kt` sharing a bare
