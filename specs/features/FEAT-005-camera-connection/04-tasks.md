...
- [ ] **T-01** — `camera/ptp/PtpFraming.kt` ← `camera/usb/ptp.ts`. `ContainerType`,
...
- [ ] **T-02** — `PtpFramingTest.kt` ← `tests/unit/ptp.spec.ts`. The two that catch real
...
- [ ] **T-03** — `camera/ptp/DeviceInfo.kt` ← `camera/usb/session.ts`. `parseDeviceInfo`
...
- [ ] **T-04** — `camera/CameraModels.kt` ← `camera/models.ts`. `MODEL_GENERATIONS`,
...
- [ ] **T-05** — `CameraModelsTest.kt`: no two names normalise to the same key; `XT5` finds
...
- [ ] **T-06** — `camera/usb/PtpTransport.kt`. The interface the session talks to:
      `command(operation, params, data): CommandResult`, `close()`. Small on purpose — it
      is what lets a fake camera stand in.
- [ ] **T-07** — `src/test/…/camera/FakeCamera.kt` ← `tests/support/fake-camera.ts`.
      Speaks PTP back: transaction ids, multi-chunk reads, stale responses, refusals.
- [ ] **T-08** — `camera/usb/UsbPtpTransport.kt` ← `camera/usb/transport.ts`. Open the
...
- [ ] **T-09** — `camera/usb/PtpSession.kt` ← `camera/usb/session.ts`. `openSession`,
      `getDeviceInfo`, `getPropValue`, `setPropValue`, `closeSession`.
- [ ] **T-10** — `PtpSessionTest.kt` against `FakeCamera`: a session opens and reads a
...
- [ ] **T-11** — `camera/CameraController.kt`. `StateFlow<CameraState>` with the six states
...
- [ ] **T-12** — `core/AppContainer.kt` gains one lazy `cameraController` field.
- [ ] **T-13** — `AndroidManifest.xml`: `<uses-feature android.hardware.usb.host
...
- [ ] **T-14** — `MainActivity` hands an attach intent to the controller so the launched-by-
      plugging path connects with no permission dialog.

## Indicator

- [ ] **T-15** — `ui/camera/CameraChip.kt` + `CameraChipLook.kt`. The six states of
...
- [ ] **T-16** — `ui/camera/CameraSheet.kt` + `CameraViewModel.kt`. Model, generation, write
...
- [ ] **T-17** — Mount the chip in `ui/shell/AppShell.kt`. Strings in `strings.xml`.
