...
- [ ] **T-01** — `camera/plan/CameraEncoding.kt` ← `camera/encoding.ts`. Property codes
...
- [ ] **T-02** — `CameraEncodingTest.kt` ← `tests/unit/camera-encoding.spec.ts`. Checked
...
- [ ] **T-03** — `camera/plan/WritePlan.kt` ← `camera/write-plan.ts`.
...
- [ ] **T-04** — `camera/ptp/StepPayload.kt` ← `camera/usb/payload.ts`. Lives beside the
...
- [ ] **T-05** — `WritePlanTest.kt`. One case per rule, named for it: WR-01 refusal with no
...
- [ ] **T-06** — `camera/usb/WriteExecutor.kt` ← `camera/usb/write.ts`. Executes steps in
...
- [ ] **T-07** — `WriteExecutorTest.kt` against `FakeCamera` (FEAT-005 T-07): a clean run
...
- [ ] **T-11** — Slot picker: seven large C1–C7 targets built from the shell's existing
...
- [ ] **T-12** — Progress: `n / total` in tabular figures with the current property named.
      Haptic `Confirm` on success and `Reject` on failure.
- [ ] **T-13** — Entry point: **Write to camera** on `ui/recipe/RecipeViewScreen.kt`'s
      floating toolbar. Disabled with a reason when no camera is connected or the body
      cannot take a write.
- [ ] **T-14** — Strings in `strings.xml`. `@Preview` for every stage, including the
